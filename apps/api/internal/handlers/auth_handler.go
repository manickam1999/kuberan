...
// GenerateMCPToken generates a long-lived token for MCP server authentication.
// @Summary     Generate MCP token
// @Description Generate a long-lived JWT token for connecting AI agents via MCP
// @Tags        auth
// @Produce     json
// @Security    BearerAuth
// @Success     200 {object} map[string]string "MCP token"
// @Failure     401 {object} ErrorResponse "Unauthorized"
// @Failure     500 {object} ErrorResponse "Server error"
// @Router      /auth/mcp-token [post]
func (h *AuthHandler) GenerateMCPToken(c *gin.Context) {
	...
	token, err := middleware.GenerateMCPToken(user)
	if err != nil {
		respondWithError(c, apperrors.Wrap(apperrors.ErrInternalServer, err))
		return
	}

	// Store the token hash so it can be validated and revoked later.
	if err := h.userService.StoreMCPTokenHash(user.ID, middleware.HashToken(token)); err != nil {
		respondWithError(c, apperrors.Wrap(apperrors.ErrInternalServer, err))
		return
	}

	h.auditService.Log(user.ID, "GENERATE_MCP_TOKEN", "user", user.ID, c.ClientIP(), nil)
...
// RevokeMCPToken clears the stored MCP token hash, invalidating any issued token.
// @Summary     Revoke MCP token
// @Description Revoke the current MCP token so it can no longer be used
// @Tags        auth
// @Produce     json
// @Security    BearerAuth
...
func (h *AuthHandler) RevokeMCPToken(c *gin.Context) {
	...
	if err := h.userService.StoreMCPTokenHash(userID, ""); err != nil {
		respondWithError(c, apperrors.Wrap(apperrors.ErrInternalServer, err))
		return
	}

	h.auditService.Log(userID, "REVOKE_MCP_TOKEN", "user", userID, c.ClientIP(), nil)
